<template>
    <div class="p-4 mt-10 w-full md:w-1/2 mx-auto">
        <div class="py-4">
            <h2 class="text-4xl md:text-5xl text-center uppercase py-1">sign up for exclusive access</h2>
            <p class="text-center capitalize">receive 10% off your first purchase plus communications about imperia products, services, stores and events.</p>
        </div>
        <div class="form-control py-2 w-full max-w-md mx-auto">
            <label class="label py-0 mt-4">
                <span class="label-text uppercase">Full Name</span>
            </label>
            <input type="text" placeholder="Full Name" class="input input-bordered w-full max-w-md" />
            <label class="label py-0 mt-4">
                <span class="label-text uppercase">Email</span>
            </label>
            <input type="email" placeholder="Email Address" class="input input-bordered w-full max-w-md" />
            <label class="label py-0 mt-4">
                <span class="label-text uppercase">Phone Number</span>
            </label>
            <input type="tel" placeholder="Phone Number" class="input input-bordered w-full max-w-md" />
            <button class="btn btn-primary my-4">Submit</button>
        </div>
    </div>
</template>